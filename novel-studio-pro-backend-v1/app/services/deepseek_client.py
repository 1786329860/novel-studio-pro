...
class DeepSeekClient:
    ...
    async def chat_stream(
        ...
        *,
        ...
        每次 yield 一个文本片段。如果 json_mode=True，会累积完整 JSON 后
        在最后一次 yield 完整的 JSON 字符串。
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=timeout) as client:
                    async with client.stream("POST", url, headers=headers, json=payload) as response:
                        if response.status_code == 429 or response.status_code >= 500:
                            raise RuntimeError(
                                f"DeepSeek 临时错误 HTTP {response.status_code}: "
                                f"{await response.aread()}"
                            )
                        if response.status_code >= 400:
                            body = await response.aread()
                            raise RuntimeError(
                                f"DeepSeek 请求失败 HTTP {response.status_code}: {body[:500]}"
                            )
        ...
        raise last_error or RuntimeError("DeepSeek 流式请求失败。")
