...
class DirectorAgent(BaseAgent):
    ...
    async def mock_run(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        # 生成场景
        scenes = []
        mood_sequence = ["平静", "微紧", "紧张", "爆发", "余韵"]
        time_sequence = ["清晨", "午后", "黄昏", "深夜", "黎明"]
    ...
    def _get_location(project: dict[str, Any], index: int) -> str:
        ...
        locations_map = {
            "奇幻": ["城门广场", "魔法塔内部", "密林深处", "古战场遗迹", "星空下营地"],
            "玄幻": ["宗门大殿", "修炼密室", "灵药园", "比武场", "悬崖边"],
            "科幻": ["空间站走廊", "实验室", "星舰驾驶舱", "废墟城市", "量子隧道"],
            "都市": ["办公室", "咖啡馆", "公寓", "停车场", "天台"],
            "悬疑": ["案发现场", "档案室", "暗巷", "审讯室", "钟楼"],
            "言情": ["校园", "图书馆", "雨中街道", "海边", "客厅"],
        }
