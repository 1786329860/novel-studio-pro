...
class WriterAgent(BaseAgent):
    ...
    def build_messages(self, context: dict[str, Any]) -> list[dict[str, str]]:
        ...
        system_prompt = (
            ...
            "14. 对话占比控制在 20%-40% 之间\n"
            "15. 角色成长：本章中至少一个主要角色必须有认知、情感或关系上的变化\n"
            "16. 对话多样性：对话必须有实质信息量。禁止超过30%的对话是谜语式/绕弯子的。角色应该有直接表达的时刻。\n"
            ...
            "23. 角色不受导演稿和蓝图角色池限制。如果剧情自然需要新角色出场，就给予足够的笔墨和深度，不要人为压缩。角色服务于故事，而非故事迁就角色列表。"
        )
    ...
    def _generate_transition(
        ...
        # 时间变化
        time_transitions = {
            ("黎明", "上午"): "天光渐亮，晨雾散去，新的一天开始了。",
            ("上午", "正午"): "日头渐渐升高，阳光变得直射而灼热。",
            ("正午", "下午"): "午后的阳光稍稍偏斜，热度却未减分毫。",
            ("下午", "傍晚"): "天边开始泛起橙红色的光，黄昏悄然降临。",
            ("傍晚", "入夜"): "最后一抹余晖消散，夜色缓缓笼罩了大地。",
            ("入夜", "深夜"): "夜已深沉，万籁俱寂，只有零星的灯火还在闪烁。",
            ("深夜", "凌晨"): "夜色最浓的时刻，时间仿佛凝固在了黎明之前。",
            ("凌晨", "黎明"): "漫长的夜终于过去，东方的天际露出一抹鱼肚白。",
            ("清晨", "中午"): "一上午的时间在忙碌中悄然流逝，转眼已是正午。",
            ("下午", "夜晚"): "暮色四合，街灯次第亮起，夜晚悄然而至。",
            ("黄昏", "子夜"): "从黄昏到子夜，时间在不知不觉中滑过了大半。",
            ("午后", "傍晚"): "午后的阳光渐渐柔和，天边染上了一层暖色。",
            ("上午", "黄昏"): "时间飞逝，等回过神来，天边已是一片橘红。",
            ("正午", "深夜"): "从正午到深夜，一整天的时间仿佛被压缩成了片刻。",
            ("傍晚", "凌晨"): "从傍晚到凌晨，夜越来越深，世界陷入沉寂。",
